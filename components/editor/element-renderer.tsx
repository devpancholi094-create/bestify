  ...
  return (
    ...
    >
      {img ? (
        ...
        <Rect width={el.width} height={el.height} fill="#efe6d3" />
      )}
      {el.borderWidth > 0 && (
        <Rect
          width={el.width}
      ...
      {showPlaceholder && <PlaceholderBadge width={el.width} height={el.height} label={el.placeholderLabel ?? "Add photo"} />}
    </Group>
  );
}

function PlaceholderBadge({ width, height, label }: { width: number; height: number; label: string }) {
  const size = Math.max(20, Math.min(width, height) * 0.22);
  return (
    <Group listening={false}>
      ...
      />
      <Path
        data="M4 12 H14 L18 6 H30 L34 12 H44 V32 H4 Z M24 22 A7 7 0 1 0 24 22.01"
        x={width / 2 - size / 2}
        y={height / 2 - size / 2 - size * 0.18}
        scaleX={size / 48}
        scaleY={size / 40}
        stroke="#a8503a"
        strokeWidth={2.5}
...
function FrameNode({ el }: { el: FrameElement }) {
  ...
  const showPlaceholder = !el.imageSrc;
    ...
    <Group>
      ...
      <Group clipX={padding} clipY={padding} clipWidth={innerW} clipHeight={innerH}>
        {img ? (
          ...
          <Rect x={padding} y={padding} width={innerW} height={innerH} fill="#efe6d3" />
        )}
      </Group>
      {showPlaceholder && (
        <Group x={padding} y={padding}>
          <PlaceholderBadge width={innerW} height={innerH} label={el.placeholderLabel ?? "Add photo"} />
        </Group>
      )}
      {el.style === "film" && (
        <>
